...
for(subject_id in 1:30) {
  for(map_id in 1:30) {
    ...
    block_1_to_5 <- floor((order_in_30 - 1) / 6) + 1
    record <- data.frame(map_id, subject_id, real_score, best_score, err_ratio, order_1_to_30, block_1_to_5)
    experiment_table <- rbind(experiment_table, record)
  }
}

experiment_table
nt <- experiment_table %>% arrange(subject_id, map_id)
nt
```
